export function LandingFeatures() {
  ...
  return (
    <section id="fitur" className="py-32 relative overflow-hidden">
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[700px] h-[500px] -z-10 opacity-40"
        style={{ background: 'radial-gradient(ellipse, rgba(99,102,241,0.08) 0%, transparent 70%)' }} />

      <div className="max-w-6xl mx-auto px-6">
        {/* Section header */}
        <div className="max-w-2xl mb-20">
          <p className="text-[11px] font-bold text-violet-400 uppercase tracking-[0.15em] mb-4">Fitur Unggulan</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
          {features.map((f) => (
            <div
              ...
              style={{
                background: `linear-gradient(135deg, ${f.accent.bg} 0%, rgba(255,255,255,0.02) 100%)`,
            ...
            >
              ...
              <div className="absolute top-0 right-0 w-48 h-48 rounded-full -z-10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
                style={{ background: `radial-gradient(circle at top right, ${f.accent.glow}, transparent 70%)` }} />

              {/* Icon + badge row */}
              <div className="flex items-center justify-between mb-5">
                <div className="flex h-10 w-10 items-center justify-center rounded-xl"
                  style={{ background: f.accent.bg, border: `1px solid ${f.accent.border}` }}>
                  {f.icon}
                </div>
                {f.badge && (
                  <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full"
                    style={{ background: f.accent.bg, color: f.accent.text, border: `1px solid ${f.accent.border}` }}>
                    {f.badge}
                  </span>
                )}
              </div>

              <h3 className="text-base font-bold text-white mb-2.5 tracking-tight">{f.title}</h3>
              <p className="text-sm text-white/45 leading-relaxed mb-4">{f.desc}</p>

              {f.tags && (
                <div className="flex flex-wrap gap-1.5">
                  {f.tags.map(tag => (
                    <span key={tag} className="text-[11px] px-2.5 py-1 rounded-full text-white/40"
